//! Evidence classifiers shared by language-specific scanners.
...
pub(super) fn check_string_for_pem(value: &str) -> bool {
    value.contains("-----BEGIN ")
}

pub(super) fn truncate_evidence(value: &str) -> Cow<'_, str> {
    match value.len() <= 40 {
...
fn check_string_for_hex_key(value: &str) -> bool {
    ...
    if length < 64 || !length.is_multiple_of(2) {
        return false;
    }
    if !value.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        return false;
    }
    matches!(length, 64 | 96) || length >= 128
}
...
fn check_string_for_base58_key(value: &str) -> bool {
    ...
    match value.as_bytes().first() {
        Some(b'5' | b'K' | b'L') if (51..=52).contains(&length) => is_base58(value),
        _ if (64..=88).contains(&length) => is_base58(value),
        _ => false,
    }
}
...
fn validate_sk_suffix(suffix: &str) -> bool {
...
fn check_string_for_credential_prefix(value: &str) -> bool {
    const CREDENTIAL_PREFIXES: &[(&str, PrefixValidator)] = &[
        ...
        ("sk_live_", |_, suffix| validate_sk_suffix(suffix)),
        ("sk_test_", |_, suffix| validate_sk_suffix(suffix)),
        ("sk-", |_, suffix| validate_sk_suffix(suffix)),
    ];

...
///
/// Three shapes at the end carry claims nothing else in the file states. The
/// `switch` binds one name twice, once per clause, so a walk that gave both
/// clauses one scope would let a sibling's binding answer for an occurrence.
/// `Sink` writes an unnamed parameter and an empty parameter list, which are
/// the two signatures a reader of bindings alone cannot tell apart. And `logf`
/// ends in a variadic parameter, which is a term of its own.
